...
There has been an active discussion on which tool to use for generating documentation. I have suggested to go with Shpinx over Doxygen, it provides an interface to integrate documentation generation with third party languages. On top of this, Grako parser generator will have to be integrated for searching text through regular expressions. Once all this is set up, one would essentially execute a `sphinx-build` command and this would parse all the source files and create a clean documentation at the end.
